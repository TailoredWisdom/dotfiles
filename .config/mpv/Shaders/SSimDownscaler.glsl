...
vec4 hook() {
    ...
    // Calculate bounds
    float low  = floor((PREKERNEL_pos - 0.5*taps*POSTKERNEL_pt) * input_size -
offset + tex_offset + 0.5)[axis];
    float high = floor((PREKERNEL_pos + 0.5*taps*POSTKERNEL_pt) * input_size -
offset + tex_offset + 0.5)[axis];
...
offset[axis]*factor;
        ...
        avg += w * pow(textureLod(PREKERNEL_raw, pos, 0.0) * PREKERNEL_mul,
vec4(2.0));
        W += w;
    }
    avg /= W;
...
vec4 hook() {
    // Calculate bounds
    float low  = floor((L2_pos - 0.5*taps*POSTKERNEL_pt) * L2_size - offset +
0.5)[axis];
    float high = floor((L2_pos + 0.5*taps*POSTKERNEL_pt) * L2_size - offset +
0.5)[axis];
...
mat2x4 ScaleH(vec2 pos) {
    ...
    for (float k = 0.0; k < maxtaps; k++) {
        ...
        avg += w * mat2x4(pow(clamp(POSTKERNEL_tex(pos), 0.0, 1.0), vec4(2.0)),
L2_tex(pos));
        W += w;
    }
    avg /= W;
...
vec4 hook() {
    ...
    vec3 Sl = clamp(Gamma(avg[0].rgb) - pow(Gamma(M_texOff(0).rgb), vec3(2.0)), 0.0,
1.0);
    vec3 Sh = clamp(Gamma(avg[1].rgb) - pow(Gamma(M_texOff(0).rgb), vec3(2.0)), 0.0,
1.0);
    return vec4(mix(vec3(0), 1.0 / (1.0 + sqrt(Sh / Sl)), lessThan(vec3(0), Sl)),
0.0);
}
