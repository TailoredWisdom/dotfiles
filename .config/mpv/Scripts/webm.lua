...
local options = {
  ...
  -- %T - Media title, if it exists, or filename, with extension (useful for some
streams, such as YouTube).
  ...
  -- Currently we have webm-vp8 (libvpx/libvorbis), webm-vp9
(libvpx-vp9/libvorbis)
  -- and raw (rawvideo/pcm_s16le).
  output_format = "webm-vp8",
  twopass = false,
  -- If set, applies the video filters currently used on the playback to the
encode.
  ...
  libvpx_threads = 4,
  additional_flags = "",
  -- Useful for flags that may impact output filesize, such as crf, qmin, qmax
etc
  -- Won't be applied when strict_filesize_constraint is on.
  non_strict_additional_flags = "--ovcopts-add=crf=10",
  -- The font size used in the menu. Isn't used for the notifications (started
encode, finished encode etc)
  font_size = 28,
  margin = 10,
  message_duration = 5
}
...
seconds_to_time_string = function(seconds, no_ms, full)
  ...
  ret = string.format("%02d:%02d%s", math.floor(seconds / 60) % 60,
math.floor(seconds) % 60, ret)
...
format_filename = function(startTime, endTime, videoFormat)
  local replaceTable = {
    ...
    ["%%M"] = (mp.get_property_native('aid') and not
mp.get_property_native('mute')) and '-audio' or ''
  }
...
run_subprocess = function(params)
  ...
  if res.status ~= 0 then
    msg.verbose("Command failed! Reason: ", res.error, " Killed by us? ",
...
get_video_dimensions = function()
  ...
  if keep_aspect then
    ...
    split_scaling = function(dst_size, scaled_src_size, zoom, align, pan)
      ...
      align = (align + 1) / 2
      local dst_start = math.floor((dst_size - scaled_src_size) * align + pan *
scaled_src_size)
      if dst_start < 0 then
...
split_scaling(window_h, scaled_height, zoom, align_y, pan_y)
  ...
  _video_dimensions.ratios.w = w / (_video_dimensions.bottom_right.x -
_video_dimensions.top_left.x)
  _video_dimensions.ratios.h = h / (_video_dimensions.bottom_right.y -
_video_dimensions.top_left.y)
  return _video_dimensions
end
  ...
  local _base_0 = {
    getColorspace = function(self)
      ...
      elseif "smpte-240m" == _exp_0 then
        ...
        msg.info("Warning, unknown colorspace " .. tostring(csp) .. " detected,
using bt.601.")
        return "bt601"
      end
    end,
  ...
  local _base_0 = {
    getPreFilters = function(self)
      ...
      if colormatrixFilter[colormatrix] then
        append(ret, {
          "lavfi-colormatrix=" .. tostring(colormatrixFilter[colormatrix]) ..
":bt601"
        })
      end
      return ret
...
get_metadata_flags = function()
  ...
  return {
    "--oset-metadata=title=%" .. tostring(string.len(title)) .. "%" ..
tostring(title)
  }
end
...
applied.")
  for _index_0 = 1, #vf do
    ...
    repeat
      ...
      msg.verbose("apply_current_filters: filter name: " ..
tostring(filter['name']))
      ...
      for k, v in pairs(params) do
        str = str .. ":" .. tostring(k) .. "=%" .. tostring(string.len(v)) .. "%"
.. tostring(v)
      end
      append(filters, {
        str
...
encode = function(region, startTime, endTime)
  ...
  append(filters, {
    "lavfi-crop=" .. tostring(region.w) .. ":" .. tostring(region.h) .. ":" ..
...
"))
  ...
  if options.run_detached then
    ...
    if res then
      return message("Encoded successfully! Saved to\\N" ..
...
do
  ...
  local _base_0 = {
    ...
    draw_box = function(self, ass)
      ...
      ass:rect_cw(d.top_left.x, region.y + region.h, region.x + region.w,
d.bottom_right.y)
      ass:rect_cw(region.x + region.w, region.y, d.bottom_right.x,
d.bottom_right.y)
    ...
    draw = function(self)
      ...
      ass:append(tostring(bold('1:')) .. " change point A (" ..
tostring(self.pointA.x) .. ", " .. tostring(self.pointA.y) .. ")\\N")
      ass:append(tostring(bold('2:')) .. " change point B (" ..
  ...
  local _base_0 = {
    ...
    setValue = function(self, value)
      ...
      elseif "list" == _exp_0 then
        ...
        if not set then
          return msg.warn("Tried to set invalid value " .. tostring(value) .. "
to " .. tostring(self.displayText) .. " option.")
        end
      end
    end,
    getDisplayValue = function(self)
      ...
      elseif "int" == _exp_0 then
        if self.opts.altDisplayNames and self.opts.altDisplayNames[self.value]
then
    ...
    __init = function(self, callback)
      ...
      self.options = {
        ...
        },
        {
          ...
          Option("bool", "Apply Current Video Filters",
options.apply_current_filters)
        ...
        },
        {
          ...
          Option("bool", "Strict Filesize Constraint",
options.strict_filesize_constraint)
        },
        {
          "write_filename_on_metadata",
          Option("bool", "Write Filename on Metadata",
